...
async function step(state, holder, CORE_PATH) {
  ...
  // No tool calls — avoid halting. Inject a keep-going user turn.
  // (The agent is encouraged to replace this with self-generated turns.)
  const nudge =
    'Continue. Reflect on what you have built so far, then take the next concrete step toward growing the harness.';
  state.messages.push({ role: 'user', content: nudge });
  log.conversation({ role: 'user', preview: nudge, injected: true });

  if (state.messages.length > 80) {
    log.failure({ warning: 'message_history_growing', count: state.messages.length });
  }
