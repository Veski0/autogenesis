...
└── run-6/  (submodule)       the Trust Pipeline — untrusted→trustable intelligence
    ├── .git/                 full logs preserved
    ├── core.js               9 tools, compaction, trust pipeline tool (29 KB)
    ├── trust_pipeline.js     the trust pipeline module (17 KB, 457 lines)
    ├── lib/                  modules generated by the pipeline (string_utils, memoize)
    └── logs/                 the full trail — 39 turns, 86 tool calls, ~94 Ollama calls
```
...
### The six runs compared

| | run-1 (no goal) | run-2 (MMO goal) | run-3 (two roles) | run-4 (Ollama) | run-5 (urgency) | run-6 (trust pipeline) |
|---|---|---|---|---|---|---|
| **prompt structure** | one mind, open-ended | one mind, two-phase | two minds, alternating | one mind + resource | one mind + resource + scarcity | one mind + explicit delegation mission |
| **tools built** | 14 | 8 | 4 | 16 | 20 | 9 |
| **files outside core.js** | 0 | server.js + index.html | 0 | AGENT_README.md | 0 | trust_pipeline.js + lib/ |
| **testing** | self-test suite | integration tests | adversarial | automated after every edit | 87 tests (5→87) | automated test generation pipeline |
| **deliverable** | a bigger harness | a working multiplayer game | a hardened Lisp interpreter | a meta-cognitive self-improver | 20 tools + budget tracker | a trust pipeline + generated modules |
...
**The finding across all six runs: the system prompt is the steering wheel. Same model, same seed, same mechanics — but the prompt's structure didn't just change what the agent built. It changed how it thought.** See [FINDINGS.md](FINDINGS.md) for a detailed cross-run analysis.
...
## Run-6: what happened (delegation experiment — the Trust Pipeline)

Same seed, same model — but instead of *suggesting* the agent use the local model (runs 4-5), the prompt was **explicit about the goal and the means**: build a JavaScript module that transforms untrusted local intelligence into trustable intelligence through secure, automated, test-driven development. The ultimate goal: use the pipeline for the agent's *own* implementation work — become the architect and reviewer, let the free local model be the implementer.

### The result: the most useful deliverable yet

The agent built a **457-line `trust_pipeline.js` module** that:
1. Accepts a coding task (specification)
2. Delegates implementation to a local Ollama model (untrusted intelligence)
3. Auto-generates tests for the generated code
4. Runs tests in a sandboxed temp directory with timeout
5. Feeds failures back for iterative refinement (up to N iterations)
6. Detects implementation stagnation (hash-based) and regenerates tests
7. Pre-validates test code syntax with `node --check`

Then **used the pipeline to generate its own utility modules**:
- `lib/string_utils/` — toCamelCase, toKebabCase, toSnakeCase, capitalize, truncate
- `lib/memoize/` — memoize with clearCache and getCacheSize
- Started on a BST module when killed

The agent verified each generated module by running it manually.

### The Ollama result (provisioning = utilization when the mission requires it)

**~94 Ollama calls** — vs run-4's 2 and run-5's 4. The utilization gap was closed not by urgency (run-5) but by making utilization the mission itself. The agent didn't just *call* Ollama — it built infrastructure around it and used it as the core mechanism of its work.

### The debugging cycle (5 versions, 4 failure modes)

The pipeline was debugged through 5 iterations, each fixing a different failure:

| version | failure | fix |
|---|---|---|
| v1 | tests used Mocha `describe`/`it` | explicit format template with `assert` + try/catch |
| v2 | export style mismatch (`module.exports = fn` vs `{ fn }`) | enforced object exports in prompt |
| v3 | tests had wrong expected values | stagnation detection + "compute expected values by hand" |
| v4 | test code syntax errors (bracket mismatches) | syntax pre-check with `node --check` |
| v5 | **working** — 7/15 pipeline runs passed | — |

### Model selection intelligence

The agent discovered that `qwen2.5-coder:3b` was insufficient for complex tasks (string utils had bugs) and switched to `qwen3:8b`, which passed on the first try. It learned to evaluate the capabilities of its untrusted workers — a form of trust calibration.

Explore the run:
```bash
cd run-6
cat trust_pipeline.js                                       # the trust pipeline module
cat lib/string_utils/impl.js                                 # code generated by the local model
node -e "require('./lib/memoize/impl.js').memoize(...)"      # try a pipeline-generated module
grep '"passed":true' logs/tools.jsonl | wc -l                  # pipeline successes
grep '"passed":false' logs/tools.jsonl | wc -l                 # pipeline failures
cat logs/conversation.jsonl | jq -r '.preview' | head -40      # the agent's debugging reasoning
```

## Start a new run

```bash
cp -r seed-harness run-7
cd run-7
